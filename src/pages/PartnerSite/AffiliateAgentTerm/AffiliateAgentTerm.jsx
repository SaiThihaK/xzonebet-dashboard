...
EditorState.createWithContent(
  ContentState.createFromBlockArray(
    convertFromHTML(`<div> ${data?.affiliate_agent}  </div> ` )
  )
  ));
  useEffect(()=>{
    setEditorState(
...
const response =await  axios.request(PutMethod(`api/xzonebet-affiliates/content`,{
  affiliate_agent:stateToHTML(editorState.getCurrentContent())
}));
...
// } 
  ...
  <PageTitleCard title="AffiliateAgent Term and Condition">
      ...
      {!open ? (
            ...
            {
                      <div className="list-style" dangerouslySetInnerHTML={{__html:   data.affiliate_agent }} />
                    }
             </p>):<CustomLoading /> ):(
...
export default AffiliateAgentTerm