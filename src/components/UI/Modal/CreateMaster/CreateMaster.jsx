...
const CreateMaster = ({ open, handleClose, render, setRender, type }) => {
  const style = {
    position: "absolute",
    top: "50%",
    left: "50%",
  ...
  const createSuperMaster = async () => {
    try {
      let apiType = "super-master";
      if (type === "provider") {
        apiType = "providers";
      }
      const response = await axios.request(PostMethod(`api/agents/${apiType}`, {
        name,
        email,
        phone,
        password,
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h6" component="h2">
            {type === "provider" ? "Create Provider" : "Create Super Master"}
          </Typography>
